...
export default function CheckoutShipping() {
  ...
  const globalState = useGlobalTransitionStates();
  ...
  const canProceedToPayment =
    ...
    !globalState.includes('submitting') &&
    !globalState.includes('loading');
  ...
  const submitAddressForm = (event: FormEvent<HTMLFormElement>) => {
    const formData = new FormData(event.currentTarget);
    const isValid = event.currentTarget.checkValidity();
    const { phoneNumber, postalCode, province } = Object.fromEntries<any>(
      formData.entries(),
    );
  ...
  return (
    <div>
      ...
      >
        ...
        {spinner && (
          <span className="h-4 w-4 border-2 border-white rounded-full border-t-blue-300 animate-spin"></span>
        )}
      </button>
    </div>
  );
